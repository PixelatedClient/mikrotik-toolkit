...
export const SUBNET_LEVELS: SubnetLevel[] = [
  ...
  {
    id: 'subnet-6',
    title: 'The Overlooked Ashes',
    story: 'A small /30 link lies forgotten in the ruins. Routers hop across it with a single gateway each. Be precise with the first and last addresses.',
    learn: 'A /30 is tiny: only 4 addresses total. The network and broadcast eat two, leaving one for each router. Use a /30 calculator to confirm the edges.',
    lives: 2,
    rounds: [
      hostsRound(2, ['/28', '/29', '/30', '/31'], '/30', '/30 has exactly 2 usable addresses, one for each router on the link.'),
      network('10.0.12.5/30', ['10.0.12.0', '10.0.12.4', '10.0.12.8', '10.0.12.12'], '10.0.12.4', 'Blocks of 4: 4 is the block that holds 5.'),
      gateway('10.0.12.4/30', ['10.0.12.0', '10.0.12.5', '10.0.12.7', '10.0.12.8'], '10.0.12.5', '4 is the network, 7 is the broadcast. 5 is usable; 6 is also usable but not an option here.'),
      bcast('192.168.254.1/30', ['192.168.254.0', '192.168.254.1', '192.168.254.2', '192.168.254.3'], '192.168.254.3', 'A /30 starting at .0 ends at .3.'),
      same('10.1.1.10', '10.1.1.25', '/27', 'Both fall in 0-31, one /27 block.'),
    ],
    read: { href: '/tools/subnet-calculator', label: 'Subnet calculator' },
  },
  {
    id: 'subnet-7',
    title: 'Supply Lines',
    story: 'The branch offices need subnets: the big HQ (300 hosts), three medium branches (120 each), and five tiny links (2 hosts each). Carve it all from 192.168.0.0/16.',
    learn: 'VLSM in action: sort by size (big to small), pick the tightest prefix for each, and align them to block boundaries. A /23 fits 510 hosts, /25 fits 126, /30 fits 2.',
    lives: 2,
    rounds: [
      hostsRound(300, ['/24', '/23', '/22', '/21'], '/23', '/23 has 510 usable addresses, enough for 300 plus growth.'),
      hostsRound(120, ['/25', '/24', '/26', '/27'], '/25', '/25 has 126 usable addresses, just enough for 120.'),
      hostsRound(14, ['/27', '/28', '/26', '/29'], '/28', '/28 has 14 usable addresses, the smallest that fits 14 devices.'),
      usable(27, ['30', '62', '126', '254'], '30', '/27 has 2^(32-27) = 32 addresses, minus 2 is 30 usable.'),
      usable(23, ['510', '254', '126', '62'], '510', '/23 has 2^(32-23) = 512 addresses, minus 2 is 510.'),
    ],
    read: { href: '/tools/vlsm-calculator', label: 'VLSM calculator' },
  },
  {
    id: 'subnet-8',
    title: 'The Careful Blocks',
    story: 'The architect is careful about boundaries. Every subnet uses a /27 for privacy barriers. Carve them from 192.168.50.0/24.',
    learn: 'A /27 block is 32 addresses. From 192.168.50.0/24 (256 total), you can fit exactly 8 blocks of /27: 0-31, 32-63, 64-95, 96-127, 128-159, 160-191, 192-223, 224-255.',
    lives: 2,
    rounds: [
      network('192.168.50.100/27', ['192.168.50.96', '192.168.50.100', '192.168.50.127', '192.168.50.128'], '192.168.50.96', 'Blocks of 32: 96 is the block that holds 100.'),
      gateway('192.168.50.32/27', ['192.168.50.32', '192.168.50.33', '192.168.50.63', '192.168.50.64'], '192.168.50.33', '32 is the network, 63 is the broadcast. 33 is the first usable.'),
      bcast('192.168.50.160/27', ['192.168.50.159', '192.168.50.160', '192.168.50.191', '192.168.50.192'], '192.168.50.191', 'The /27 block starting at 160 ends at 191.'),
      same('192.168.50.70', '192.168.50.90', '/27', 'Both fall in 64-95, one /27 block.'),
    ],
    read: { href: '/tools/subnet-calculator', label: 'Subnet calculator' },
  },
  {
    id: 'subnet-9',
    title: 'Boss: The Endless Road',
    boss: 'The Endless Road',
    story: 'One final test in the hidden chamber. The riddles come fast, mixing everything: tiny links, huge networks, three octets changing at once.',
    learn: 'Master every trick: /30 links, /16 supernets, /27 tight blocks, and /24 offices. Block boundaries, network/broadcast edges, and host counts all at once.',
    lives: 2,
    rounds: [
      usable(16, ['65534', '65530', '32766', '32768'], '65534', '/16 has 2^16 = 65536 addresses, minus 2 is 65534.'),
      network('172.31.99.50/16', ['172.31.0.0', '172.30.0.0', '172.32.0.0', '172.31.99.0'], '172.31.0.0', 'A /16 boundary is on the third octet. 172.31.anything is 172.31.0.0.'),
      same('10.100.1.5', '10.100.1.200', '/24', 'Both in 10.100.1.x, so they share a /24.'),
      hostsRound(500, ['/24', '/23', '/22', '/21'], '/23', '/23 has 2^9 = 512 minus 2 = 510 usable, enough for 500 devices.'),
      gateway('192.168.0.128/25', ['192.168.0.128', '192.168.0.129', '192.168.0.255', '192.168.0.127'], '192.168.0.129', '128 is the network, 255 is the broadcast. 129 is the first usable address.'),
      bcast('10.0.255.1/24', ['10.0.255.0', '10.0.255.255', '10.1.0.0', '10.0.254.255'], '10.0.255.255', 'The broadcast of 10.0.255.0/24 is 10.0.255.255.'),
      network('172.16.128.99/25', ['172.16.128.0', '172.16.128.128', '172.16.129.0', '172.16.127.128'], '172.16.128.0', 'Blocks of 128: 0, 128, 256... 99 falls in the first block at 0.'),
      hostsRound(2, ['/29', '/30', '/31', '/32'], '/30', 'A /30 has 4 addresses, minus 2 is 2 usable, perfect for a router link.'),
    ],
    read: { href: '/practice/subnetting', label: 'Subnetting practice' },
  },
];
