...
export const FW_LEVELS: FwLevel[] = [
  ...
  },
  {
    id: 'fw-11',
    title: 'DNS Lockdown',
    story: 'The company DNS runs on 192.168.88.5:53. Staff (10.0.0.0/8) can query it with UDP and TCP. Outside queries and local probes from other machines must be blocked.',
    learn: 'DNS uses UDP port 53 (queries) and TCP port 53 (zone transfers). Allow it only from trusted sources to your DNS server. Replies come back via the established rule.',
    goal: 'Allow DNS queries and transfers from 10.0.0.0/8 to the DNS server, block outside access and other local access.',
    packets: [
      P('Office DNS query', { chain: 'forward', iface: 'lan', src: '10.1.1.50', dst: '192.168.88.5', proto: 'udp', dstPort: 53, evil: false, why: 'Office staff DNS query.' }),
      P('DNS reply', { chain: 'forward', state: 'established', src: '192.168.88.5', dst: '10.1.1.50', proto: 'udp', dstPort: 51000, evil: false, iface: 'lan', why: 'DNS response.' }),
      P('Outside DNS query', { chain: 'forward', dst: '192.168.88.5', proto: 'udp', dstPort: 53, evil: true, src: '203.0.113.8', why: 'Nobody outside should query internal DNS.' }),
      P('Other LAN host probing DNS', { chain: 'forward', iface: 'lan', src: '192.168.88.20', dst: '192.168.88.5', proto: 'tcp', dstPort: 53, evil: true, why: 'Not from the office network.' }),
      P('Office TCP zone transfer', { chain: 'forward', iface: 'lan', src: '10.2.2.100', dst: '192.168.88.5', proto: 'tcp', dstPort: 53, evil: false, why: 'Zone transfer from office network.' }),
    ],
    par: 3,
    solution: [
      { chain: 'forward', action: 'accept', state: ['established', 'related'] },
      { chain: 'forward', action: 'accept', proto: 'udp', src: '10.0.0.0/8', dstPort: [53], dst: '192.168.88.5/32', iface: 'lan' },
      { chain: 'forward', action: 'accept', proto: 'tcp', src: '10.0.0.0/8', dstPort: [53], dst: '192.168.88.5/32', iface: 'lan' },
    ],
    fields: ['chain', 'action', 'proto', 'port', 'src', 'dst', 'state', 'iface'],
    read: { href: '/learn/mikrotik/05-firewall-and-nat', label: 'Firewall and NAT rules' },
  },
