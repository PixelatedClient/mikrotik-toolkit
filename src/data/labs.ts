...
export const LABS: Lab[] = [
  ...
  {
    id: 'ospf-chain',
    title: 'OSPF chain topology and accumulated costs',
    level: 'Intermediate',
    minutes: 40,
    summary: 'Four routers in a line with OSPF. Learn how costs accumulate across multiple hops.',
    objectives: ['Build OSPF in a linear topology', 'Understand accumulated costs across multiple hops', 'Learn about router-ids and loopbacks'],
    nodes: [
      { id: 'PC1', label: 'PC1', kind: 'pc', x: 50, y: 130 },
      { id: 'R1', label: 'R1', kind: 'router', x: 150, y: 130 },
      { id: 'R2', label: 'R2', kind: 'router', x: 295, y: 130 },
      { id: 'R3', label: 'R3', kind: 'router', x: 440, y: 130 },
      { id: 'R4', label: 'R4', kind: 'router', x: 580, y: 130 },
      { id: 'PC4', label: 'PC4', kind: 'pc', x: 610, y: 130 },
    ],
    links: [
      { a: 'PC1', ai: 'eth0', b: 'R1', bi: 'ether3', net: '192.168.10.0/24' },
      { a: 'R1', ai: 'ether1', b: 'R2', bi: 'ether1', net: '10.0.12.0/30' },
      { a: 'R2', ai: 'ether2', b: 'R3', bi: 'ether1', net: '10.0.23.0/30' },
      { a: 'R3', ai: 'ether2', b: 'R4', bi: 'ether1', net: '10.0.34.0/30' },
      { a: 'R4', ai: 'ether3', b: 'PC4', bi: 'eth0', net: '192.168.40.0/24' },
    ],
    setup: [
      'Browser-only lab: four routers in a line topology, run entirely in the simulator.',
      'All devices are pre-wired and ready for configuration. Start with loopbacks and link addresses.',
    ],
    tasks: [
      { title: 'Configure OSPF instances and areas', detail: 'Set up OSPF on all four routers with loopback addresses as router-ids and the backbone area.' },
      { title: 'Add interface templates', detail: 'Configure all four point-to-point links with cost 10 each, and mark LANs and loopbacks as passive.' },
      { title: 'Verify path and costs', detail: 'Confirm that R1 learns the route to LAN4 via R2 and R3, with an accumulated cost of 31 (10+10+10+1).' },
      { title: 'Failover on link break', detail: 'Disable the R1-R2 link. Verify that R1 loses all OSPF neighbours and cannot reach LAN4.' },
      { title: 'Restore connectivity', detail: 'Re-enable the R1-R2 link and confirm neighbours and routes return instantly (in the simulator).' },
    ],
    verify: [
      { node: 'R1', title: 'OSPF instances', cmd: '/routing ospf instance print', expect: 'ospf1 instance with router-id 10.255.0.1' },
      { node: 'R1', title: 'Neighbours formed', cmd: '/routing ospf neighbor print', expect: 'One neighbour in Full state (R2)' },
      { node: 'R1', title: 'Routes learned', cmd: '/ip route print where ospf', expect: 'Route to 192.168.40.0/24 with cost 31, via 10.0.12.2' },
    ],
    related: [
      { href: '/learn/routing/04-ospf-for-enterprise', label: 'OSPF for Enterprise' },
      { href: '/learn/foundations/07-tcpip-in-action', label: 'TCP/IP in action' },
    ],
  },
  {
    id: 'ospf-mesh',
    title: 'OSPF mesh topology with redundancy',
    level: 'Intermediate',
    minutes: 50,
    summary: 'Four routers in a square with a diagonal. Learn path selection in a mesh with expensive backup links.',
    objectives: ['Set up OSPF in a mesh topology', 'Understand path selection with multiple equal paths', 'Learn failover with redundant paths'],
    nodes: [
      { id: 'PC1', label: 'PC1', kind: 'pc', x: 50, y: 80 },
      { id: 'R1', label: 'R1', kind: 'router', x: 150, y: 80 },
      { id: 'R2', label: 'R2', kind: 'router', x: 580, y: 80 },
      { id: 'R3', label: 'R3', kind: 'router', x: 150, y: 180 },
      { id: 'R4', label: 'R4', kind: 'router', x: 580, y: 180 },
      { id: 'PC4', label: 'PC4', kind: 'pc', x: 100, y: 220 },
    ],
    links: [
      { a: 'PC1', ai: 'eth0', b: 'R1', bi: 'ether3', net: '192.168.1.0/24' },
      { a: 'R1', ai: 'ether1', b: 'R2', bi: 'ether1', net: '10.0.12.0/30' },
      { a: 'R1', ai: 'ether2', b: 'R3', bi: 'ether1', net: '10.0.13.0/30' },
      { a: 'R1', ai: 'ether4', b: 'R4', bi: 'ether3', net: '10.0.14.0/30 (expensive)' },
      { a: 'R2', ai: 'ether2', b: 'R4', bi: 'ether2', net: '10.0.24.0/30' },
      { a: 'R3', ai: 'ether2', b: 'R4', bi: 'ether1', net: '10.0.34.0/30' },
      { a: 'R3', ai: 'ether3', b: 'PC4', bi: 'eth0', net: '192.168.4.0/24' },
    ],
    setup: [
      'Browser-only lab: four routers in a square with diagonal mesh topology, run entirely in the simulator.',
      'All devices are pre-wired. The expensive diagonal link R1-R4 provides backup redundancy.',
    ],
    tasks: [
      { title: 'Full OSPF setup', detail: 'Create OSPF instances with loopback router-ids, backbone area, and interface templates for all five links (square edges cost 10, diagonal R1-R4 costs 100).' },
      { title: 'Verify neighbours and path', detail: 'R1 should have 3 neighbours, confirm it reaches LAN4 via R3 (cost 21), not via the expensive diagonal (cost 101).' },
      { title: 'First failover', detail: 'Disable R1-R3 link. R1 now takes R1→R2→R4 path (same cost 21). Traceroute shifts from going through R3 to going through R2.' },
      { title: 'Second failover', detail: 'Disable R1-R2 as well. Now only the diagonal remains (cost 101). R1 is left with just one OSPF neighbour (R4).' },
      { title: 'Restore redundancy', detail: 'Re-enable both R1-R3 and R1-R2. Neighbours return, cheaper paths reappear, and traffic reverts to the preferred R3 path.' },
    ],
    verify: [
      { node: 'R1', title: 'OSPF instances', cmd: '/routing ospf instance print', expect: 'ospf1 instance with router-id 10.255.0.1' },
      { node: 'R1', title: 'Neighbours at mesh', cmd: '/routing ospf neighbor print', expect: 'Three neighbours in Full state' },
      { node: 'R1', title: 'Preferred path', cmd: '/ip route print where ospf', expect: 'Route to 192.168.4.0/24 via 10.0.13.2 (R3), cost 21' },
    ],
    related: [
      { href: '/learn/routing/04-ospf-for-enterprise', label: 'OSPF for Enterprise' },
      { href: '/learn/foundations/07-tcpip-in-action', label: 'TCP/IP in action' },
