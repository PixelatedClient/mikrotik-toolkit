...
export const LABS: Lab[] = [
  ...
  {
    id: 'ospf-mesh',
    title: 'OSPF square mesh: 4 routers connected in a square',
    level: 'Advanced',
    minutes: 50,
    summary: 'Four routers in a square (R1-R2-R4-R3 forming a loop) with a cross-link from R1 to R4. Configure OSPF and watch how it chooses paths through a mesh.',
    objectives: ['Build OSPF on a square mesh topology', 'Understand SPF calculation with multiple equal-cost paths', 'See how OSPF breaks ties and selects best paths', 'Measure convergence when paths change'],
    nodes: [
      { id: 'R1', label: 'R1', kind: 'router', x: 100, y: 100, file: 'R1.rsc' },
      { id: 'R2', label: 'R2', kind: 'router', x: 250, y: 100, file: 'R2.rsc' },
      { id: 'R4', label: 'R4', kind: 'router', x: 250, y: 250, file: 'R4.rsc' },
      { id: 'R3', label: 'R3', kind: 'router', x: 100, y: 250, file: 'R3.rsc' },
      { id: 'PC1', label: 'PC1', kind: 'pc', x: 40, y: 40 },
      { id: 'PC4', label: 'PC4', kind: 'pc', x: 310, y: 310 },
    ],
    links: [
      { a: 'PC1', ai: 'eth0', b: 'R1', bi: 'ether3', net: '192.168.1.0/24' },
      { a: 'R1', ai: 'ether1', b: 'R2', bi: 'ether1', net: '10.0.12.0/30 (cost 10)' },
      { a: 'R2', ai: 'ether2', b: 'R4', bi: 'ether2', net: '10.0.24.0/30 (cost 10)' },
      { a: 'R4', ai: 'ether1', b: 'R3', bi: 'ether2', net: '10.0.34.0/30 (cost 10)' },
      { a: 'R3', ai: 'ether1', b: 'R1', bi: 'ether2', net: '10.0.13.0/30 (cost 10)' },
      { a: 'R1', ai: 'ether4', b: 'R4', bi: 'ether3', net: '10.0.14.0/30 (cost 100)' },
      { a: 'R3', ai: 'ether3', b: 'PC4', bi: 'eth0', net: '192.168.4.0/24' },
    ],
    setup: [
      'Add four CHR routers and two VPCS nodes in GNS3 and wire them in a square. R1 is top-left, R2 is top-right, R4 is bottom-right, R3 is bottom-left.',
      'Add a diagonal link from R1 to R4 with high cost (100). This creates a mesh where R1-R2-R4 path costs 20, R1-R3-R4 path costs 20, and the diagonal costs 100.',
      'Paste each config. Wait about 40 seconds for neighbours to reach Full.',
      'On the PCs run: ip 192.168.1.10/24 192.168.1.1 (PC1) and ip 192.168.4.10/24 192.168.4.1 (PC4).',
    ],
    tasks: [
      { title: 'Bring up OSPF', detail: 'Create instances and backbone area on all four routers. Then add templates for all five links.' },
      { title: 'Count the neighbours', detail: 'Check how many neighbours each router has: R1 (3), R2 (2), R3 (2), R4 (3). All should be Full.' },
      { title: 'Predict the path', detail: 'From R1 to 192.168.4.0/24 (R4\'s LAN), which way is cheaper: R1→R2→R4 (cost 20), R1→R3→R4 (cost 20), or R1→R4 direct (cost 100)? Confirm with traceroute.' },
      { title: 'Break one path', detail: 'Disable R1 ether1 (link to R2). Now only R1→R3→R4 works. Traceroute should still reach PC4.' },
      { title: 'Break another path', detail: 'Disable R1 ether2 (link to R3) as well. Now only the expensive R1→R4 diagonal works. Traceroute should now use it.' },
      { title: 'Restore and restore again', detail: 'Re-enable R1 ether2 and verify the path changes back to R1→R3→R4. Then re-enable ether1 and confirm it goes back to the original path.' },
    ],
    verify: [
      { node: 'R1', title: 'Neighbours', cmd: '/routing ospf neighbor print', expect: 'Three neighbours: R2, R3, R4 all Full.' },
      { node: 'R2', title: 'Neighbours', cmd: '/routing ospf neighbor print', expect: 'Two neighbours: R1, R4 both Full.' },
      { node: 'R1', title: 'Path to PC4 LAN', cmd: '/ip route print where ospf', expect: 'Route to 192.168.4.0/24 via 10.0.12.2 (R2) or 10.0.13.2 (R3), cost 21, not via the diagonal (cost 101).' },
    ],
    related: [
      { href: '/learn/routing/04-ospf-for-enterprise', label: 'OSPF for enterprise networks' },
      { href: '/learn/routing/01-bgp-basics', label: 'BGP Basics' },
    ],
  },
  {
    id: 'ospf-chain',
